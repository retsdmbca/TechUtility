...
Function Run {
    if ($RadioButton1.Checked -eq $true) {Start-Process powershell.exe -ArgumentList '-noprofile -file C:\ProgramData\RETSD\TechUtility.ps1' }
    if ($RadioButton2.Checked -eq $true) {Start-Process powershell.exe -ArgumentList '-noprofile -file C:\ProgramData\RETSD\TechUtility.ps1' -Verb RunAs}
}
